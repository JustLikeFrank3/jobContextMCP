...
(Tauri/pywebview) uses these to manage the backend it spawned:
    ...
    POST /desktop/shutdown — stop the server gracefully.  Fallback for shells
    that can't signal the sidecar (and the #1 defence against orphaned
    backend processes holding the SQLite file).

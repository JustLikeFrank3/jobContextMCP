...
Every generation and AI-edit path runs a deterministic truth gate (`lib/provenance.py`): numeric claims in a draft (percentages, dollar amounts, magnitudes, years) must exist in the run's source material — master resume, STAR stories, retrieved chunks, JD — or the agent pipeline routes back for revision. LLM reviewer approval alone is never sufficient. Every run writes an audit record (`generation_provenance`); edits to the master resume itself are audited too (`master_resume_edits`), so the gate's ground truth can't drift invisibly. The verdict surfaces as a one-line summary in every confirmation and a pass/fail badge in the dashboard; failures open a violations modal with per-claim **Fix** / **Fix all** actions that hand off to the AI-edit flow (which re-runs the gate).

## Prompt budgeting & story retrieval

Prompt assembly is retrieval-first: fixed context sections are bounded, tone samples are packed by recency/diversity, the remaining personal-story budget is computed dynamically, and a final prompt ceiling is enforced before calling the model. Cover-letter generation pulls relevant personal stories from your personal context library; for mission/brand-heavy roles it uses semantic story retrieval (cached embeddings + hook-tag boosts) so abstract company language finds the strongest human angle instead of only matching literal keywords. Scraped JDs are cleaned before prompting so LinkedIn navigation chrome doesn't crowd out the actual posting.

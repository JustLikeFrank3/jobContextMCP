...
**Metric discontinuity at 2026-08-14 (stories join the judge's evidence).** The master-context bundle the judge reads now includes the personal-story library as a STORIES section, and the excerpt cap was raised so nothing is truncated away. Before this date the judge never saw stories, so every story-sourced claim in a generated document was flagged as a hallucination on every run — the same failure family as the 6K-truncation incident, discovered when a chunk-count figure that lives in a story was flagged for weeks straight. **Hallucination rates and flagged-claim counts before and after this boundary are not comparable**: an entry's rate dropping across it measures the judge's evidence set expanding, not the generator improving. The judge's rubric, prompt wording, model, and pass criterion are unchanged — same referee, fuller case file. Corollary: story facts are now citable, so keeping stories true (counts and dates drift — see the volatile-source taxonomy in the triage notes) is part of maintaining the ground truth, exactly like master-resume edits.

**Reverted-master windows (through 2026-08-22): ground truth was intermittently corrupt.** Until the seed-workspace no-clobber fix landed (PR #264/#265, 2026-08-22), every pod restart ran an unconditional blob restore over the data volume, silently reverting any master-resume edit made in the ~15 minutes before it — the 2026-08-01 placement corrections (Azure App Insights and the MSAL/OAuth2 documentation bullet, both moved from Level 5 to Level 6) were lost this way twice. **Runs judged while the master was in a reverted state measured corrupted ground truth**: the generator faithfully reproduced the master's own misplaced layout and was flagged for it against the master's guardrail note (run 363, 2026-08-22, is the documented example — a large share of its placement-class flags trace to this). Every component behaved correctly over wrong data. Flag counts and accuracy scores from suite runs stored on or before 2026-08-22 are therefore not comparable to later runs and must not be read as a generation-quality trend across that boundary; the stored payloads are kept, marked by this note, not deleted. The first run on durable, fully repaired ground truth is the 2026-08-25 nightly (the 24th is a Sunday; cadence is Mon/Wed/Fri) — though the on-demand run stored 2026-08-23T04:29 already measured the repaired state.

**Per-tenant eval loop (`/dashboard/evals`, `evals/tenant.py`).** Every partition can author its own golden set (paste 3–5 real JDs on the screen; JD files land in `workspace/evals/golden/`, manifest in `workspace/evals/golden_dataset.json`), run the suite against its own master, triage flagged claims A/B/C/D with notes (rulings persist across runs, keyed by entry + exact claim text), and choose its own judge. A tenant judge preference (`workspace/evals/judge.json`, key write-only) applies to that partition's runs only and every surface labels the judge with its measured calibration status — an uncalibrated judge's scores are comparable to its own earlier runs, never across judges. The committed manifest and env judge remain the owner-partition fallback. The D ruling ("true but undocumented") is the loop's core insight productized: most surviving flags in the owner's own runs were true facts missing from ground truth, and the fix is documentation, not suppression.
...
For the wallboard touchscreen — where nobody is logged into anything — the Pi's app instance serves a human-readable version at **`GET /wallboard/evals`**: per-entry scores, the alert strings in words, the flagged claims quoted, plus the work-stats token table. The Pi fetches the payload server-side with an owner-minted PAT (`PI_WALLBOARD_EVALS_PAT` in `.env.pi` → `WALLBOARD_EVALS_PAT`); the viewer never authenticates. The route 404s wherever that env pair is absent — i.e. on every cloud and desktop deployment — so the login-free exposure of eval detail (which names real employers in claim text) exists only on a LAN box whose operator explicitly configured it. Payload text is HTML-escaped wholesale: the claims are LLM output being rendered into a page, hostile markup until proven otherwise.

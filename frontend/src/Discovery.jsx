...
function Review() {
  ...
  return <>
    ...
    {people.map(p => { const sessions = ledger.sessions.filter(s => s.participant_id === p.id); const incentives = ledger.incentives.filter(i => i.participant_id === p.id); return <details key={p.id} className="discovery-person">
      <summary><strong>{p.name}</strong> · {p.contact} <span>#{p.id} / {p.status} / {(p.programs || [p.program]).join(' + ')}</span></summary>
      <p>{p.created_at} · {friendly(p.segment)} · {friendly(p.channel)}</p>
      <dl>{Object.entries(p.screener || {}).filter(([k]) => k !== 'program_signups').map(([k, v]) => <React.Fragment key={k}><dt>{friendly(k)}</dt><dd>{Array.isArray(v) ? v.join(', ') : v}</dd></React.Fragment>)}</dl>
      {Object.entries(p.screener?.program_signups || {}).map(([program, signup]) => <section key={program}><h3>{friendly(program)} signup</h3><p>{signup.created_at} · {friendly(signup.channel)}</p><dl>{Object.entries(signup.screener).map(([key, value]) => <React.Fragment key={key}><dt>{key === 'best_window' && program === 'beta' ? 'Testing availability' : friendly(key)}</dt><dd>{Array.isArray(value) ? value.join(', ') : value || 'Not provided'}</dd></React.Fragment>)}</dl></section>)}

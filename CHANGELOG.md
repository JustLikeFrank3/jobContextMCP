...
- **`cli.py` — direct tool invocation from terminal**: `python3 cli.py <tool> [json_kwargs]` dispatches to any of the 52 registered MCP tools without an AI client; supports `--list` to enumerate all tools with signatures; eliminates the temp-script workaround for scripted updates and development workflows.

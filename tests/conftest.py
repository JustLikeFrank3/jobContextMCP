...
# ──────────────────────────────────────────────────────────────────────────────
# LLM guard — prevents any test from making real LLM / Ollama network calls.
# Tag a test with @pytest.mark.live_llm to opt out (integration tests only).
# ──────────────────────────────────────────────────────────────────────────────

_FAKE_RESUME = (
    "FRANK MACBRIDE\nfrank@example.com\n\n"
    "SUMMARY\n• Test resume generated by mock.\n\n"
    "EXPERIENCE\n• Built things. | Test Co, Remote | Jan 2024 - Present\n"
)
_FAKE_COVER = (
...
def _mock_llm(request, monkeypatch):
    """Patch generate_resume + generate_cover_letter for every test unless
    the test is marked @pytest.mark.live_llm."""
    if request.node.get_closest_marker("live_llm"):
        yield
        return
    import tools.generate as gen
    monkeypatch.setattr(gen, "generate_resume", lambda *a, **kw: _FAKE_RESUME)
    monkeypatch.setattr(gen, "generate_cover_letter", lambda *a, **kw: _FAKE_COVER)
    # Also patch the local reference imported into the LangGraph resume graph
    try:
        import workflows.langgraph.resume_graph as rg
        monkeypatch.setattr(rg, "generate_resume", lambda **_kw: _FAKE_RESUME)
    except ImportError:
        pass
    yield

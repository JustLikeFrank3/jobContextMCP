"""Resume and cover-letter text parsers.

Parses the strict .txt format produced by generate_resume / generate_cover_letter
into structured dicts consumed by the PDF rendering layer (tools/export.py).

All public symbols are also re-exported from tools/export.py so existing
callers that import directly from export are unaffected.
"""
